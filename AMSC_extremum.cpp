...
template<typename T>
void AMSC<T>::ComputeMaximumGraph(boost::numeric::ublas::matrix<int> &edges)
{
  map_pi_pfi pinv;
  for(int i = 0; i < Size(); i++)
  {
...
template<typename T>
void AMSC<T>::ComputeMinimumGraph(boost::numeric::ublas::matrix<int> &edges)
{
  //initial persistences
  //store as pairs of extrema such that p.first merges to p.second (e.g.
  //p.second is the min with the smaller function value
